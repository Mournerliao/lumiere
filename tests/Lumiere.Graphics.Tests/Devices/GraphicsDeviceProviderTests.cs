...
namespace Lumiere.Graphics.Tests.Devices;

public sealed class GraphicsDeviceProviderTests
{
    [Fact]
    public void DefaultOptionsEnableBgraSupportForWinUiAndDxgiInterop()
    {
    ...
    public void NativeInteropFailureMapsToInteropReadinessStage()
    {
        ...
        Assert.Equal(PreviewReadinessState.Failed, status.State);
        Assert.Equal(PreviewReadinessStage.Interop, status.Stage);
