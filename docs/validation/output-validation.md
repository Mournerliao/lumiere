...
As of the current implementation, Lumiere has a Windows WIC-backed JPEG XR adapter for `64bppRGBAHalf` byte streams and the app wires the HDR10 JXR codec through that adapter. The adapter embeds Lumiere audit metadata under `Lumiere:*` XMP query paths so emitted artifacts can carry the selected HDR10 policy values for inspection. Runtime HDR10 export remains disabled because this audit metadata is not yet proven equivalent to viewer-recognized HDR10 static metadata, and Windows manual viewer validation gates have not passed.
