...
- [ ] Confirm Story 1.1 scaffold prerequisites before implementation. (AC: 1, 2, 3)
  - [ ] Verify `Lumiere.sln`, `Directory.Build.props`, `Directory.Packages.props`, and `src/Lumiere.Graphics/` exist from Story 1.1.
  - [ ] Verify the app targets `net10.0-windows10.0.19041.0`, `x64`, Windows App SDK `1.8.260317003`, `Vortice.Direct3D11` `3.8.3`, and `Vortice.DXGI` `3.8.3`.
  - [ ] If the scaffold is missing, stop and complete Story 1.1 first instead of inventing a different project layout.
- [ ] Add centralized HDR constants in the graphics boundary. (AC: 1)
  - [ ] Create `src/Lumiere.Graphics/Hdr/HdrConstants.cs` or the closest existing `Lumiere.Graphics` namespace/folder pattern.
  - [ ] Expose the WGC frame-pool pixel format as `DirectXPixelFormat.R16G16B16A16Float`.
  - [ ] Expose the DXGI swap-chain format as `Format.R16G16B16A16_Float` / `DXGI_FORMAT_R16G16B16A16_FLOAT` using the Vortice type already referenced by the project.
  - [ ] Expose the DXGI color space as `ColorSpaceType.RgbFullG10NoneP709` / `DXGI_COLOR_SPACE_RGB_FULL_G10_NONE_P709` using the Vortice type already referenced by the project.
  - [ ] Keep the constants strongly typed; do not store these values as strings, magic integers, app settings, or loose duplicated literals.
- [ ] Define typed preview readiness status for early pipeline state. (AC: 3)
  - [ ] Add a compact status model under `src/Lumiere.Graphics/Hdr/` or a shared diagnostics/status namespace if Story 1.1 already established one.
  - [ ] Include at least `Unknown` or `Initializing`, `Ready`, `Degraded`, `Unsupported`, and `Failed` states so future capture/graphics stories can distinguish "not yet validated" from "validated HDR-ready".
  - [ ] Include stage/detail fields or a companion record for capture, graphics, presentation, overlay, interop, or lifecycle diagnostics when readiness cannot be established.
  - [ ] Ensure degraded/unsupported/failed states can carry a user-facing message and technical detail without requiring UI code to inspect D3D/DXGI objects.
- [ ] Add tests that lock the HDR constants and status behavior. (AC: 2, 3)
  - [ ] Add or extend a test project that follows the scaffolded repository pattern, preferably `tests/Lumiere.Graphics.Tests/`.
  - [ ] Assert the WGC pixel format remains `DirectXPixelFormat.R16G16B16A16Float`.
  - [ ] Assert the swap-chain format remains the FP16 format and cannot drift to `B8G8R8A8`, `R8G8B8A8`, or other 8-bit SDR-oriented formats.
  - [ ] Assert the color space remains the scRGB linear HDR color space and cannot drift to an SDR color space.
  - [ ] Assert an unvalidated preview path reports a typed non-ready state such as `Degraded` or `Unsupported`, not `Ready` and not a silent fallback.
- [ ] Integrate without implementing later graphics/capture behavior. (AC: 1, 3)
  - [ ] Reference `HdrConstants` from any existing placeholder graphics status code if it exists, but do not implement D3D11 device creation, WGC frame pools, swap-chain attachment, or live preview in this story.
  - [ ] Keep UI/app shell changes minimal; readiness may be represented as a typed model now and rendered in later stories.
  - [ ] Do not add export, clipboard, hotkey, tray, annotation, history, or SDR screenshot fallback behavior.
- [ ] Validate the story output. (AC: 1, 2, 3)
  - [ ] Run formatting for touched files.
  - [ ] Run `dotnet restore` on `Lumiere.sln`.
  - [ ] Run `dotnet build Lumiere.sln -p:Platform=x64`.
  - [ ] Run the HDR constants/readiness tests.
  - [ ] Review the final diff and explicitly call out that HDR constants or readiness semantics were touched.
