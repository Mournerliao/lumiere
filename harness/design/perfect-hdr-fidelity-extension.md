...
- Viewer compatibility rows should keep category evidence explicit in readable text, separating artifact handling, visual match, HDR preservation, HDR10 metadata, and target-app version evidence instead of compressing everything into one color-only badge or one opaque paragraph.
