...
Workspace-local companion evidence is now part of that loading contract. If an artifact lists an `evidence\...` path, Lumiere resolves it inside the same local validation workspace and ignores the JSON artifact when the markdown notes, screenshots, logs, or other referenced file are missing. This keeps a generated or edited JSON file from advancing `Build` / `Validate` / `Ready` projections when its Story `12-1` scenario-session notes have been moved, deleted, or never filled in. Repo-relative references such as `docs/validation/evidence/...` remain review references; the app only enforces paths that are explicitly local to the validation workspace.
