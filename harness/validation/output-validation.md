...
Workspace-local companion evidence is now part of that loading contract. If an artifact lists an `evidence\...` path, Lumiere resolves it inside the same local validation workspace and ignores the JSON artifact when the markdown notes, screenshots, logs, or other referenced file are missing. Workspace-local markdown evidence must also be filled in: empty markdown, `REPLACE_WITH_*` placeholders, or `Template only` language make the JSON artifact a load issue instead of current-session evidence. This keeps a generated or edited JSON file from advancing `Build` / `Validate` / `Ready` projections when its Story `12-1` scenario-session notes have been moved, deleted, or left as a template. Repo-relative references such as `docs/validation/evidence/...` remain review references; the app only enforces paths that are explicitly local to the validation workspace.
...
- the companion markdown scenario-session draft is also a placeholder until a Windows validator fills in observed scenario results, evidence paths, and limitations
